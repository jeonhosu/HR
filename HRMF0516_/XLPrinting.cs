...
namespace HRMF0516
{
    public class XLPrinting
    {
        ...
        public void HeaderWrite(InfoSummit.Win.ControlAdv.ISDataCommand pPrinted_Value, object pPAY_YYYYMM, object vWAGE_TYPE_NAME, object vWAGE_TYPE)
        {// 헤더 인쇄.
            ...
            try
            {
                ...
                {
                    vPrinted_Value = string.Format("[Salary Period : {0} ]", vPrinted_Value );
                }
                else
                {
        ...
        #region ----- Excel Write [Line] Method -----0

        private int LineWrite(InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid, int pGridRow, int pXLine, int[] pGDColumn, int[] pXLColumn, object pAllowance_type)
        ...
        public int ExcelWrite(InfoSummit.Win.ControlAdv.ISDataCommand pPrinted_Value, InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid, object vPay_YYYYMM, object vWAGE_TYPE_NAME, object vWAGE_TYPE)
        {// 실제 호출되는 부분.

            string vMessage = string.Empty;
            object Pay_YYYYMM = null;
            object WAGE_TYPE_NAME = null;
            object WAGE_TYPE = null;
            object vAllowance_type = null;
            ...
            try
            {
                HeaderWrite(pPrinted_Value, vPay_YYYYMM, vWAGE_TYPE_NAME, vWAGE_TYPE);
                ...
                if (vTotalRow > 0)
                {
                    ...
                    for (int vRow = 0; vRow < vTotalRow; vRow++)
                    {
                        ...
                        mCurrentRow = LineWrite(pGrid, vRow, mCurrentRow, vGDColumn, vXLColumn, vAllowance_type); // 현재 위치 인쇄 후 다음 인쇄행 리턴.
