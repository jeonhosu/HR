...
namespace HRMF0771
{
    public partial class HRMF0771 : Office2007Form
    {
        #region ----- Variables -----
        ...
        public HRMF0771(Form pMainForm, ISAppInterface pAppInterface)
        {
            InitializeComponent();
        ...
        private void isAppInterfaceAdv1_AppMainButtonClick(ISAppButtonEvents e)
        {
            if (this.IsActive)
            {
                ...
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.AddOver)
                {
                    ...
                    {
                        IDA_RESIDENT_BUSINESS.AddOver();
                        NAME.Focus();
                    }
                    else if (IDA_RESIDENT_BSN_FAMILY.IsFocused)
                    {
                        IDA_RESIDENT_BSN_FAMILY.AddOver();
                        IGR_RESIDENT_BSN_FAMILY.Focus();
                    }
                }
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.AddUnder)
                {
                    if (IDA_RESIDENT_BUSINESS.IsFocused)
                    {
                        IDA_RESIDENT_BUSINESS.AddUnder();
                        NAME.Focus();
                    }
                    else if (IDA_RESIDENT_BSN_FAMILY.IsFocused)
                    {
                        IDA_RESIDENT_BSN_FAMILY.AddUnder();
                        IGR_RESIDENT_BSN_FAMILY.Focus();
                    }
                }
                ...
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Cancel)
                {
                    if (IDA_RESIDENT_BUSINESS.IsFocused)
                    {
                        IDA_RESIDENT_BSN_FAMILY.Cancel();
                        IDA_RESIDENT_BUSINESS.Cancel();
                    }
                    else if (IDA_RESIDENT_BSN_FAMILY.IsFocused)
                    {
                        IDA_RESIDENT_BSN_FAMILY.Cancel();
                ...
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Delete)
                {
                    ...
                    //}
                    //else 
                    if (IDA_RESIDENT_BSN_FAMILY.IsFocused)
                    {
                        IDA_RESIDENT_BSN_FAMILY.Delete();
                    }
                }
        ...
        private void HRMF0771_Load(object sender, EventArgs e)
        {
            IDA_RESIDENT_BUSINESS.FillSchema();
            IDA_RESIDENT_BSN_FAMILY.FillSchema();
        }

        private void HRMF0771_Shown(object sender, EventArgs e)
        {
            DefaultCorporation();
        ...
        private void IDA_RESIDENT_BSN_FAMILY_PreRowUpdate(ISPreRowUpdateEventArgs e)
        {
            if (iConv.ISNull(e.Row["NAME"]) == string.Empty)
            {
                MessageBoxAdv.Show(isMessageAdapter1.ReturnText("EAPP_90004", string.Format("&&FIELD_NAME:={0}", Get_Edit_Prompt(NAME))), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            ...
            if (iConv.ISNull(e.Row["RELATION_ID"]) == string.Empty)
            {
                MessageBoxAdv.Show(isMessageAdapter1.ReturnText("EAPP_90004", "&&FIELD_NAME:=Relation(包拌)"), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                e.Cancel = true;
                return;
            }
            if (iConv.ISNull(e.Row["NATIONALITY_TYPE"]) == string.Empty)
