...
namespace HRMF0513
{
    public class XL_Upload
    {
        ...
        public bool LoadXL_10(InfoSummit.Win.ControlAdv.ISDataAdapter pAdapter, int pStartRow)
        {
            ...
            object vASSET_DESC = string.Empty;

            int vADRow = 0;
            int vADCol = 0;

            mTotalCOLUMN = pAdapter.OraSelectData.Columns.Count - 1;  //-2의미는 아답터에 기본 2COLUMN이 포함되어 있으므로(ROWKEYID, ROW STATUS) //
            ...
            try
            {
                for (int vRow = pStartRow; vRow < mTotalROW; vRow++)
                {
                    ...
                    {
                        vASSET_DESC = mExcel_Upload.XLGetCell(vRow, 1);  //KEY 값.
                    }
                    else
                    {
                        vASSET_DESC = string.Empty;
                        pAdapter.Delete();
                    }
                    if (iString.ISNull(vASSET_DESC).Trim() != string.Empty)  //사원번호가 있을 경우만 처리.
                    {
        ...
        public bool LoadXL_20(InfoSummit.Win.ControlAdv.ISDataAdapter pAdapter, int pStartRow)
        {
            string vMessage = string.Empty;
            ...
            try
            {
                for (int vRow = pStartRow; vRow < mTotalROW; vRow++)
                {
                    pAdapter.AddUnder();
                    //KEY값에 해당하는 셀에 DATA가 있을 경우만 INSERT를 처리해야 하므로//
                    vType = pAdapter.CurrentRow.Table.Columns["CHARGE_CODE"].DataType;
                    if (vType.Name == "String")
                    {
                        vCHARGE_CODE = mExcel_Upload.XLGetCell(vRow, 4);  //KEY 값.
                    }
                    else
                    {
                        vCHARGE_CODE = string.Empty;
                        pAdapter.Delete();
                    }
                    if (iString.ISNull(vCHARGE_CODE).Trim() != string.Empty)  //자산명 있을 경우만 처리.
                    {
                        for (int vCol = 1; vCol < mTotalCOLUMN; vCol++)
                        {
                            vType = pAdapter.CurrentRow.Table.Columns[vADCol].DataType;
