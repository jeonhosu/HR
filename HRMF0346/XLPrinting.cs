...
namespace HRMF0346
{
    ...
    public class XLPrinting
    {
        ...
        private int mMaxIncrement = 38; //실제 출력되는 행의 시작부터 끝 행의 범위
        ...
        private void XLHeader(string pTitle, string pHeaderLeft, string pHeaderRight)
        {
            try
            {
                ...
                //XLHeaderRight(6, 53, pHeaderRight); //상단의 오른쪽
            }
            catch (System.Exception ex)
            {
                mMessageError = string.Format("{0}", ex.Message);
        ...
        private void XLContentWrite(XL.XLPrint pPrinting, InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid, int pTotalRow, int pXLine, int[] pXLColumn, int pPrintingRowSTART, int pPrintingRowEND)
        {
            ...
            try
            {
                ...
                // 신청자 데이터 삽입
                int vIndexDataColumn = pGrid.GetColumnToIndex("REQUEST_PERSON_NAME");
                vObject = pGrid.GetCellValue(vIndexDataColumn);
                pPrinting.XLSetCell(6, pXLColumn[9], vObject);
