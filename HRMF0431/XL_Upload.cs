...
namespace HRMF0431
{
    public class XL_Upload
    {
        ...
        public bool LoadXL(InfoSummit.Win.ControlAdv.ISDataCommand pCMD, int pStartRow, InfoSummit.Win.ControlAdv.ISProgressBar pPB, InfoSummit.Win.ControlAdv.ISPrompt pPM)
        {
            ...
            object vObject = null;
            object vYEAR_YYYY = string.Empty;
            object vNAME = string.Empty;
            object vPERSON_NUM = string.Empty;
            //DateTime vTRANSFER_DATE;
            ...
            try
            {
                for (int vRow = pStartRow; vRow < mTotalROW; vRow++)
                {
                    ...
                    vYEAR_YYYY = mExcel_Upload.XLGetCell(vRow, 1);
                    ...
                    if (iString.ISNull(vPERSON_NUM) != string.Empty)  //사원번호가 있을 경우만 처리.
                    {
                        try
                        {
                            ...
                            //vTRANSFER_DATE =  ConvertDate(mExcel_Upload.XLGetCell(vRow, 3)); 

                            pCMD.SetCommandParamValue("P_YEAR_YYYY", vYEAR_YYYY);
                            pCMD.SetCommandParamValue("P_PERSON_NUM", vPERSON_NUM);
                            pCMD.SetCommandParamValue("P_NAME", vNAME);
                            pCMD.SetCommandParamValue("P_YEAR_INSUR_AMT", mExcel_Upload.XLGetCell(vRow, 4)); 
                            pCMD.SetCommandParamValue("P_PRE_PAYMENT_INSUR_AMT", mExcel_Upload.XLGetCell(vRow, 5));
                            pCMD.SetCommandParamValue("P_YEAR_ADJ_INSUR_AMT", mExcel_Upload.XLGetCell(vRow, 6));
                            pCMD.SetCommandParamValue("P_TOT_ADJ_INSUR_AMT", mExcel_Upload.XLGetCell(vRow, 7));
                            pCMD.SetCommandParamValue("P_DIST_MONTH", mExcel_Upload.XLGetCell(vRow, 8));  
                            ...
                            if (iString.ISNull(pCMD.GetCommandParamValue("O_STATUS")) == "F")
                            {
                                ...
                                pPM.PromptText = string.Format("Importing :: {0} / {1} *** Name : {2}({3} ** Error : {4})", 
                                                                vRow, (mTotalROW - pStartRow), vNAME, vPERSON_NUM, vMessage);
                                vERR_CNT++;
                                return false;
                            }
                        }
