                ...
                {                    
                    ...
                    if (iString.ISNull(vPERSON_NUM) != string.Empty)  //사원번호가 있을 경우만 처리.
                    {
                        try
                        {
                            ...
                            pCMD.SetCommandParamValue("P_TRANSFER_DATE", vTRANSFER_DATE); 
                            pCMD.SetCommandParamValue("P_TRANSFER_AMOUNT", mExcel_Upload.XLGetCell(vRow, 4));
                            pCMD.SetCommandParamValue("P_DESCRIPTION", mExcel_Upload.XLGetCell(vRow, 5)); 
