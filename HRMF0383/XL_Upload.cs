                ...
                {                    
                    ...
                    if (iString.ISNull(vPERSON_NUM) != string.Empty)  //사원번호가 있을 경우만 처리.
                    {
                        try
                        {
                            ...
                            pCMD.SetCommandParamValue("P_DESCRIPTION", mExcel_Upload.XLGetCell(vRow, 6));
                    ...
                    pPM.PromptText = string.Format("Imporing :: {0}-{1} *** {2}({3})", vADRow, (mTotalROW - 1 - pStartRow), vNAME, vPERSON_NUM);

                    vMessage = string.Format("Excel Uploading : {0:D4}/{1:D4}", vRow, ((mTotalROW - 1 - pStartRow)));
                    mAppInterface.OnAppMessage(vMessage);
                    System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.WaitCursor;
                    System.Windows.Forms.Application.DoEvents();
                }
