...
namespace HRMF0573
{
    public partial class HRMF0573 : Office2007Form
    {
        #region ----- Variables -----
        ...
        public HRMF0573(Form pMainForm, ISAppInterface pAppInterface)
        {
            InitializeComponent();
        ...
        private void HRMF0573_Load(object sender, EventArgs e)
        {
            DefaultCorporation();
            // Default_Floor(); 
        }

        private void HRMF0573_Shown(object sender, EventArgs e)
        {
        ...
        private void BTN_OK_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            IDC_SET_ALLOWANCE_OP_INSA.ExecuteNonQuery();
            string vSTATUS = iConv.ISNull(IDC_SET_ALLOWANCE_OP_INSA.GetCommandParamValue("O_STATUS"));
            string vMESSAGE = iConv.ISNull(IDC_SET_ALLOWANCE_OP_INSA.GetCommandParamValue("O_MESSAGE"));
            if (vSTATUS == "F")
            {
        ...
        private void BTN_CANCEL_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            IDC_SET_ALLOWANCE_OP_INSA_CANCEL.ExecuteNonQuery();
            string vSTATUS = iConv.ISNull(IDC_SET_ALLOWANCE_OP_INSA_CANCEL.GetCommandParamValue("O_STATUS"));
            string vMESSAGE = iConv.ISNull(IDC_SET_ALLOWANCE_OP_INSA_CANCEL.GetCommandParamValue("O_MESSAGE"));
            if (vSTATUS == "F")
            {
                if (vMESSAGE != string.Empty)
                {
        ...
        private void ILA_ALLOWANCE_CODE_PrePopupShow(object pSender, ISLookupPopupShowEventArgs e)
        {
        ...
        private void IDA_ALLOWANCE_OP_PreRowUpdate(ISPreRowUpdateEventArgs e)
        {
            ...
            if (iConv.ISNull(e.Row["OP_RATE_CODE"]) == string.Empty)
            {
                e.Cancel = true;
                object vPrompt = Get_Grid_Prompt(IGR_ALLOWANCE_OP, IGR_ALLOWANCE_OP.GetColumnToIndex("ALLOWANCE_OP_RATE_NAME"));
                MessageBoxAdv.Show(isMessageAdapter1.ReturnText("EAPP_90004", String.Format("&&FIELD_NAME:={0}", vPrompt)), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            if (iConv.ISNull(e.Row["ALLOWANCE_ID"]) == string.Empty)
            {
                e.Cancel = true;
                object vPrompt = Get_Grid_Prompt(IGR_ALLOWANCE_OP, IGR_ALLOWANCE_OP.GetColumnToIndex("ALLOWANCE_NAME"));
                MessageBoxAdv.Show(isMessageAdapter1.ReturnText("EAPP_90004", String.Format("&&FIELD_NAME:={0}", vPrompt)), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        }
