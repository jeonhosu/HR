...
namespace HRMF0776
{
    public class XLPrinting
    {
        ...
        private int mCopy_EndCol = 48;
        private int mCopy_EndRow = 33;
        private int mPrintingLastRow = 32;  //실제 데이터 인쇄 최종 라인.
        ...
        {// 그리드의 컬럼에 대한 컬럼인덱스 값 산출
            ...
            // 그리드 or 아답터 위치.
            pGDColumn[0] = pGrid.GetColumnToIndex("CORP_NAME");
            pGDColumn[1] = pGrid.GetColumnToIndex("EARNER_NUM");
            pGDColumn[2] = pGrid.GetColumnToIndex("NAME");
            pGDColumn[3] = pGrid.GetColumnToIndex("REPRE_NUM");
            pGDColumn[4] = pGrid.GetColumnToIndex("PAY_DATE");
            pGDColumn[5] = pGrid.GetColumnToIndex("RECEIPT_DATE");
            pGDColumn[6] = pGrid.GetColumnToIndex("PAYMENT_AMOUNT");
            pGDColumn[7] = pGrid.GetColumnToIndex("TAX_RATE");
            pGDColumn[8] = pGrid.GetColumnToIndex("INCOME_TAX_AMT");
            pGDColumn[9] = pGrid.GetColumnToIndex("LOCAL_TAX_AMT");
            pGDColumn[10] = pGrid.GetColumnToIndex("TOTAL_DED_AMT");
            pGDColumn[10] = pGrid.GetColumnToIndex("REAL_AMT");
        ...
        public void HeaderWrite(object pPeriod_Date)
        {// 헤더 인쇄.
            ...
            try
            {
                ...
                vXLine = 3;
                vXLColumn = 40;
                mPrinting.XLSetCell(vXLine, vXLColumn, pPeriod_Date);
            }
            catch (System.Exception ex)
            {
                mMessageError = ex.Message;
        ...
        {// pGridRow : 그리드의 현재 읽는 행, pXLine : 엑셀의 인쇄해야 하는 행
            ...
            { // 원본을 복사해서 타겟 에 복사해 넣음.(
                ...
                if (iString.ISDecimal(vObject) != false)
                {
                    vConvertString = string.Format("{0:###,###,###,###,###,###,###,###}", vObject);
                }
                else
        ...
        public int ExcelWrite(object pPeriod_Date, InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid)
        {// 실제 호출되는 부분.
