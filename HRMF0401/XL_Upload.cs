...
namespace HRMF0401
{
    public class XL_Upload
    {
        ...
        public bool LoadXL_P(InfoSummit.Win.ControlAdv.ISDataCommand pCMD, int pStartRow, InfoSummit.Win.ControlAdv.ISProgressBar pPB, InfoSummit.Win.ControlAdv.ISPrompt pPM)
        {
            ...
            try
            {
                for (int vRow = pStartRow; vRow < mTotalROW; vRow++)
                {
                    ...
                    if (iString.ISNull(vPERSON_NUM) != string.Empty)  //사원번호가 있을 경우만 처리.
                    {
                        try
                        {
                            pCMD.SetCommandParamValue("P_INSUR_TYPE", "P");
                            pCMD.SetCommandParamValue("P_PERSON_NUM", vPERSON_NUM);
                            pCMD.SetCommandParamValue("P_NAME", vNAME);
                            pCMD.SetCommandParamValue("P_INSUR_YN", mExcel_Upload.XLGetCell(vRow, 3));
                            pCMD.SetCommandParamValue("P_INSUR_NO", mExcel_Upload.XLGetCell(vRow, 4));
                            pCMD.SetCommandParamValue("P_CORP_INSUR_AMOUNT", mExcel_Upload.XLGetCell(vRow, 5)); 
                            pCMD.SetCommandParamValue("P_PERSON_INSUR_AMOUNT", mExcel_Upload.XLGetCell(vRow, 6)); 
                            pCMD.SetCommandParamValue("P_DESCRIPTION", mExcel_Upload.XLGetCell(vRow, 7));
                            pCMD.SetCommandParamValue("P_GET_DATE", mExcel_Upload.XLGetCell(vRow, 8));
                            pCMD.SetCommandParamValue("P_LOSS_DATE", mExcel_Upload.XLGetCell(vRow, 9));
                            pCMD.SetCommandParamValue("P_INSUR_GRADE_STEP", mExcel_Upload.XLGetCell(vRow, 10));
                            pCMD.ExecuteNonQuery();
                            if (pCMD.ExcuteError)
                            {
                                vErrorMessage = iString.ISNull(pCMD.ExcuteErrorMsg);
                                vERR_CNT++;
        ...
        public bool LoadXL_M(InfoSummit.Win.ControlAdv.ISDataCommand pCMD, int pStartRow, InfoSummit.Win.ControlAdv.ISProgressBar pPB, InfoSummit.Win.ControlAdv.ISPrompt pPM)
        {
            string vMessage = string.Empty;
            string vErrorMessage = string.Empty;
            ...
            try
            {
                for (int vRow = pStartRow; vRow < mTotalROW; vRow++)
                {
                    ...
                    if (iString.ISNull(vPERSON_NUM) != string.Empty)  //사원번호가 있을 경우만 처리.
                    {
                        try
                        {
                            pCMD.SetCommandParamValue("P_INSUR_TYPE", "M");
                            pCMD.SetCommandParamValue("P_PERSON_NUM", vPERSON_NUM);
                            pCMD.SetCommandParamValue("P_NAME", vNAME);
                            pCMD.SetCommandParamValue("P_INSUR_YN", mExcel_Upload.XLGetCell(vRow, 3));
                            pCMD.SetCommandParamValue("P_INSUR_NO", mExcel_Upload.XLGetCell(vRow, 4));
                            pCMD.SetCommandParamValue("P_CORP_INSUR_AMOUNT", mExcel_Upload.XLGetCell(vRow, 5));
                            pCMD.SetCommandParamValue("P_CORP_LTC_INSUR_AMOUNT", mExcel_Upload.XLGetCell(vRow, 6));
                            pCMD.SetCommandParamValue("P_PERSON_INSUR_AMOUNT", mExcel_Upload.XLGetCell(vRow, 7));
                            pCMD.SetCommandParamValue("P_PERSON_LTC_INSUR_AMOUNT", mExcel_Upload.XLGetCell(vRow, 8));
                            pCMD.SetCommandParamValue("P_DESCRIPTION", mExcel_Upload.XLGetCell(vRow, 9));
                            pCMD.SetCommandParamValue("P_GET_DATE", mExcel_Upload.XLGetCell(vRow, 10));
                            pCMD.SetCommandParamValue("P_LOSS_DATE", mExcel_Upload.XLGetCell(vRow, 11));
                            pCMD.SetCommandParamValue("P_INSUR_GRADE_STEP", mExcel_Upload.XLGetCell(vRow, 12)); 
                            pCMD.ExecuteNonQuery();
                            if (pCMD.ExcuteError)
                            {
                                vErrorMessage = iString.ISNull(pCMD.ExcuteErrorMsg);
