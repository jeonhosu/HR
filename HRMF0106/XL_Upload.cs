    ...
    public class XL_Upload
    {
        ...
        public bool LoadXL(InfoSummit.Win.ControlAdv.ISDataCommand pCMD, int pStartRow, InfoSummit.Win.ControlAdv.ISProgressBar pPB, InfoSummit.Win.ControlAdv.ISPrompt pPM, object pSTD_DATE)
        {
            ...
            try
            {
                ...
                {                    
                    //KEY값에 해당하는 셀에 DATA가 있을 경우만 INSERT를 처리해야 하므로//
                    vObject = iString.ISNull(mExcel_Upload.XLGetCell(vRow, 1)).Replace("-", "");  //월급여액 START  
                    ...
                    vObject = iString.ISNull(mExcel_Upload.XLGetCell(vRow, 2)).Replace("-", "");  //월급여액 END  
                    if (iString.ISDecimal(vObject) == true)
                    {
                    ...
                    try
                    {
                        vObject = iString.ISNull(mExcel_Upload.XLGetCell(vRow, 3)).Replace("-", "");  //부양가족1.
                    ...
                    try
                    {
                        vObject = iString.ISNull(mExcel_Upload.XLGetCell(vRow, 4)).Replace("-", "");  //부양가족2
