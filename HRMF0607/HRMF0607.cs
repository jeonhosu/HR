...
namespace HRMF0607
{
    public partial class HRMF0607 : Office2007Form
    {
        ...
        public HRMF0607(Form pMainForm, ISAppInterface pAppInterface)
        {
            InitializeComponent();
        ...
        private void SearchDB()
        {
            ...
            if (iDate.ISGetDate(END_DATE_0.EditValue) < iDate.ISGetDate(START_DATE_0.EditValue))
            {
                MessageBoxAdv.Show(isMessageAdapter1.ReturnText("FCM_10012"), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                START_DATE_0.Focus();
                return;
            }
            if (iDate.ISYear(START_DATE_0.EditValue) != iDate.ISYear(END_DATE_0.EditValue))
            {
                MessageBoxAdv.Show("시작일자와 종료일자가 다른 년도입니다. 확인하세요", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                START_DATE_0.Focus();
                return;
            }
            YYYY_0.EditValue = iDate.ISYear(END_DATE_0.EditValue);
            IDA_eFILE_INFO.Fill();
        }
        ...
        private string EXPORT_VALIDATE()
        {
            ...
            if (IGR_RETIRE_FILE_LIST.RowCount < 1)
            {
        ...
        private void Button_Control(string pEnabled_YN)
        {
            if (pEnabled_YN == "Y")
            {
                BTN_RETIRE_FILE.Enabled = true;
            }
            else
            {
                BTN_RETIRE_FILE.Enabled = false;
            }
        ...
        private void ExportTXT(string pFileName, ISDataAdapter pData)
        {
            ...
            HRMF0607_FILE vHRMF0607_FILE = new HRMF0607_FILE(isAppInterfaceAdv1.AppInterface);
            vdlgResult = vHRMF0607_FILE.ShowDialog();
            if (vdlgResult == DialogResult.OK)
            {
                vENCRYPT_PASSWORD = vHRMF0607_FILE.Get_Encrypt_Password;
            }
        ...
        private void ExportTXT_BAK(string pFileName, ISDataAdapter pData)
        {
            object vFIX_STRING = null;
            int vCountRow = pData.OraSelectData.Rows.Count;
            if (vCountRow < 1)
        ...
        private void HRMF0607_Load(object sender, EventArgs e)
        {

        }

        private void HRMF0607_Shown(object sender, EventArgs e)
        {
            DefaultCorporation();

            //기준일자.
            YYYY_0.EditValue = iDate.ISYear(DateTime.Today);
            START_DATE_0.EditValue = iDate.ISMonth_1st(string.Format("{0}-01", iDate.ISYear(DateTime.Today)));
            END_DATE_0.EditValue = iDate.ISMonth_Last(string.Format("{0}-12", DateTime.Today.Year));

            YYYY_0.Focus();
            WRITE_DATE.EditValue = DateTime.Today;
        }

        private void IGR_RETIRE_FILE_LIST_CurrentCellChanged(object pSender, ISGridAdvExChangedEventArgs e)
        {
            if (e.RowIndex < 0)
            {
                return;
            }
            IGR_RETIRE_FILE_LIST.LastConfirmChanges();
            IDA_RETIRE_FILE_LIST.OraSelectData.AcceptChanges();
            IDA_RETIRE_FILE_LIST.Refillable = true;
        }

        private void START_DATE_0_CurrentEditValidated(object pSender, ISEditAdvValidatedEventArgs e)
        {
            END_DATE_0.EditValue = string.Format("{0}-12-31", iDate.ISYear(START_DATE_0.EditValue));
        }


        private void BTN_RETIRE_FILE_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            int vIDX_SELECT_YN = IGR_RETIRE_FILE_LIST.GetColumnToIndex("SELECT_YN");
            int vIDX_OPERATING_UNIT_ID = IGR_RETIRE_FILE_LIST.GetColumnToIndex("OPERATING_UNIT_ID");
            int vIDX_VAT_NUMBER = IGR_RETIRE_FILE_LIST.GetColumnToIndex("VAT_NUMBER");
            int vIDX_TAX_OFFICE_CODE = IGR_RETIRE_FILE_LIST.GetColumnToIndex("TAX_OFFICE_CODE");
            int vIDX_REC_NOW_PERSON_COUNT = IGR_RETIRE_FILE_LIST.GetColumnToIndex("REC_NOW_PERSON_COUNT");
            int vIDX_REC_PRE_PERSON_COUNT = IGR_RETIRE_FILE_LIST.GetColumnToIndex("REC_PRE_PERSON_COUNT");
            for (int r = 0; r < IGR_RETIRE_FILE_LIST.RowCount; r++)
            {
                if (iConv.ISNull(IGR_RETIRE_FILE_LIST.GetCellValue(r, vIDX_SELECT_YN)) == "Y")
                {
                    IGR_RETIRE_FILE_LIST.CurrentCellMoveTo(r, vIDX_SELECT_YN);
                    ...
                    object vOPERATING_UNIT_ID = IGR_RETIRE_FILE_LIST.GetCellValue(r, vIDX_OPERATING_UNIT_ID);
                    string vVAT_NUMBER = iConv.ISNull(IGR_RETIRE_FILE_LIST.GetCellValue(r, vIDX_VAT_NUMBER));
                    object vTAX_OFFICE_CODE = IGR_RETIRE_FILE_LIST.GetCellValue(r, vIDX_TAX_OFFICE_CODE);
                    ...
                    IDA_RETIRE_FILE.SetSelectParamValue("P_OPERATING_UNIT_ID", vOPERATING_UNIT_ID);
                    IDA_RETIRE_FILE.Fill();
                    ExportTXT(vVAT_NUMBER, IDA_RETIRE_FILE);

                    //생성된 자료수 체크//
                    IDC_GET_RETIRE_FILE_P.SetCommandParamValue("P_OPERATING_UNIT_ID", vOPERATING_UNIT_ID);
                    IDC_GET_RETIRE_FILE_P.ExecuteNonQuery();
                    object vREC_NOW_COUNT = IDC_GET_RETIRE_FILE_P.GetCommandParamValue("O_REC_NOW_COUNT");
                    object vREC_PRE_COUNT = IDC_GET_RETIRE_FILE_P.GetCommandParamValue("O_REC_PRE_COUNT");

                    IGR_RETIRE_FILE_LIST.SetCellValue(r, vIDX_REC_NOW_PERSON_COUNT, vREC_NOW_COUNT);
                    IGR_RETIRE_FILE_LIST.SetCellValue(r, vIDX_REC_PRE_PERSON_COUNT, vREC_PRE_COUNT);
                    IGR_RETIRE_FILE_LIST.SetCellValue(r, vIDX_SELECT_YN, "N");
                }
            } 
            IGR_RETIRE_FILE_LIST.LastConfirmChanges();
            IDA_RETIRE_FILE_LIST.OraSelectData.AcceptChanges();
        ...
        private void IDA_RETIRE_FILE_LIST_PreNewRowMoved(object pSender, ISBindingEventArgs pBindingManager)
        {
            if (pBindingManager.DataRow == null)
            {
                return;
            ...
            {
                IGR_RETIRE_FILE_LIST.GridAdvExColElement[vIDX_SELECT_YN].Insertable = 0;
                IGR_RETIRE_FILE_LIST.GridAdvExColElement[vIDX_SELECT_YN].Updatable = 0;
            }
            else
            {
                IGR_RETIRE_FILE_LIST.GridAdvExColElement[vIDX_SELECT_YN].Insertable = 1;
                IGR_RETIRE_FILE_LIST.GridAdvExColElement[vIDX_SELECT_YN].Updatable = 1;
            }
        }
