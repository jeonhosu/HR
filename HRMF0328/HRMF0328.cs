...
namespace HRMF0328
{
    public partial class HRMF0328 : Office2007Form
    {
        #region ----- Variables -----
        ...
        public HRMF0328(Form pMainForm, ISAppInterface pAppInterface)
        {
            InitializeComponent();
        ...
        private void HRMF0328_Load(object sender, EventArgs e)
        {
            IDA_PERSON_WEEK_40H.FillSchema();
        }

        private void HRMF0328_Shown(object sender, EventArgs e)
        {
            DefaultValues();

            DUTY_YYYYMM_0.EditValue = iDate.ISYearMonth(DateTime.Today);

            IDC_USER_CAP_R.SetCommandParamValue("W_START_DATE", DateTime.Today);
            IDC_USER_CAP_R.SetCommandParamValue("W_END_DATE", DateTime.Today);
            IDC_USER_CAP_R.SetCommandParamValue("W_MODULE_CODE", "20");
            IDC_USER_CAP_R.ExecuteNonQuery();
            string vCAP_LEVEL = iConv.ISNull(IDC_USER_CAP_R.GetCommandParamValue("O_CAP_LEVEL"));
            if(vCAP_LEVEL == "C")
            {
                BTN_SET_OT.Visible = true;
            }
            else
            {
                BTN_SET_OT.Visible = false;
            } 
        }
        ...
        private void isButton1_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            IDC_SET_OT.ExecuteNonQuery();
            mStatus = IDC_SET_OT.GetCommandParamValue("O_STATUS").ToString();
            mMessage = iString.ISNull(IDC_SET_OT.GetCommandParamValue("O_MESSAGE"));
            Application.UseWaitCursor = false;
            this.Cursor = Cursors.Default;
            Application.DoEvents();

            if (IDC_SET_OT.ExcuteError || mStatus == "F")
            {
