        ...
        public bool LoadXL(InfoSummit.Win.ControlAdv.ISDataAdapter pAdapter, int pStartRow)
        {
            ...
            try
            {
                for (int vRow = pStartRow; vRow < mTotalROW; vRow++)
                {
                    ...
                    vType = pAdapter.CurrentRow.Table.Columns["WORK_DATE"].DataType;
                    if (vType.Name == "DateTime")
                    {
                        vWORK_DATE = mExcel_Upload.XLGetCell(vRow, 1);  //KEY 값.
                    }
                    else
                    {
                        vWORK_DATE = string.Empty;
                        pAdapter.Delete();
                    }
                    if (iString.ISNull(vWORK_DATE).Trim() != string.Empty)  //근무일 있을 경우만 처리.
                    {
