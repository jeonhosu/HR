...
namespace HRMF0107
{
    public partial class HRMF0107_OLD : Office2007Form
    {
        ...
        public HRMF0107_OLD(Form pMainForm, ISAppInterface pAppInterface)
        {
            InitializeComponent();
        ...
        private void SEARCH_DB()
        {
            ...
            mYYYY = Convert.ToString(Convert.ToInt32(STD_YEAR_0.EditValue));
            idcTAX_STANDARD_CHECK_YN.SetCommandParamValue("W_YEAR_YYYY", mYYYY);
            idcTAX_STANDARD_CHECK_YN.ExecuteNonQuery();
            mReturn_Value = iString.ISNull(idcTAX_STANDARD_CHECK_YN.GetCommandParamValue("O_CHECK_YN"));
            ...
            {// 기존 자료가 존재하지 않을 경우.
                ...
                // 기존에 검색된 값을 제거하고 초기화 상태로 셋팅하는 함수 호출.                        
                // allTextBoxValueDeleteSetting();
                return;
            }
            //-----------------------------------------------------------------------------------------------------
        ...
        private void isAppInterfaceAdv1_AppMainButtonClick(ISAppButtonEvents e)
        {
            if (this.IsActive)
            {
                ...
                {
                    // 데이터 등록-AddOver
                    if (idaINCOME_TAX_STANDARD.IsFocused)
                    {
                        idaINCOME_TAX_STANDARD.AddOver();
                        payDeleteDefaultSetting();
                    }                                        
                }
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.AddUnder)
                {
                    ...
                    if (idaINCOME_TAX_STANDARD.IsFocused)
                    {
                        idaINCOME_TAX_STANDARD.AddUnder();
                        payDeleteDefaultSetting();
                    }                                        
                }
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Update)
                {
                    idaINCOME_TAX_STANDARD.Update();
                }
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Cancel)
                {
                    if (idaINCOME_TAX_STANDARD.IsFocused)
                    {
                        idaINCOME_TAX_STANDARD.Cancel();
                        payDefaultSetting();
                    }
                }
        ...
        */
        ...
        {
            idaINCOME_TAX_STANDARD.FillSchema();            
        }
        ...
        private void ibtCOPY_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            mMESSAGE = iString.ISNull(idcTAX_STANDARD_COPY.GetCommandParamValue("O_CHECK_YN"));
