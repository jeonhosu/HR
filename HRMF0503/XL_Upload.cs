...
namespace HRMF0503
{
    public class XL_Upload
    {
        ...
        public bool LoadXL_Allowance(InfoSummit.Win.ControlAdv.ISDataCommand pCMD, int pStartRow, InfoSummit.Win.ControlAdv.ISProgressBar pPB, InfoSummit.Win.ControlAdv.ISPrompt pPM)
        {
            string vMessage = string.Empty;
            ...
            object vPAY_YYYYMM = string.Empty;
            object vWAGE_TYPE = string.Empty;
            object vPERSON_NUM = string.Empty;
            object vNAME = string.Empty;
            object vALLOWANCE_CODE = string.Empty;
            object vALLOWANCE_NAME = string.Empty;
            ...
            try
            {
                ...
                {
                    //KEY값에 해당하는 셀에 DATA가 있을 경우만 INSERT를 처리해야 하므로//
                    vPAY_YYYYMM = mExcel_Upload.XLGetCell(vRow, 1);
                    vWAGE_TYPE = mExcel_Upload.XLGetCell(vRow, 2);
                    vPERSON_NUM = mExcel_Upload.XLGetCell(vRow, 4); 

                    if (iString.ISNull(vPAY_YYYYMM) != string.Empty &&
                        iString.ISNull(vWAGE_TYPE) != string.Empty &&
                        iString.ISNull(vPERSON_NUM) != string.Empty)  //사원번호가 있을 경우만 처리.
                    {
                        try
                        {
                            vNAME = mExcel_Upload.XLGetCell(vRow, 5);
                            vALLOWANCE_CODE = mExcel_Upload.XLGetCell(vRow, 6);
                            vALLOWANCE_NAME = mExcel_Upload.XLGetCell(vRow, 7);

                            pCMD.SetCommandParamValue("P_PAY_YYYYMM", vPAY_YYYYMM);
                            pCMD.SetCommandParamValue("P_WAGE_TYPE", vWAGE_TYPE); 
                            pCMD.SetCommandParamValue("P_WAGE_TYPE_NAME", mExcel_Upload.XLGetCell(vRow, 3));
                            pCMD.SetCommandParamValue("P_PERSON_NUM", vPERSON_NUM);
                            pCMD.SetCommandParamValue("P_NAME", vNAME);
                            pCMD.SetCommandParamValue("P_ALLOWANCE_CODE", vALLOWANCE_CODE);
                            pCMD.SetCommandParamValue("P_ALLOWANCE_NAME", vALLOWANCE_NAME);
                            pCMD.SetCommandParamValue("P_ALLOWANCE_AMOUNT", mExcel_Upload.XLGetCell(vRow, 8));
                            pCMD.SetCommandParamValue("P_TAX_YN", mExcel_Upload.XLGetCell(vRow, 9));
                            pCMD.SetCommandParamValue("P_HIRE_INSUR_YN", mExcel_Upload.XLGetCell(vRow, 10));
                            pCMD.SetCommandParamValue("P_DESCRIPTION", mExcel_Upload.XLGetCell(vRow, 11)); 
                            pCMD.ExecuteNonQuery();
                            if (iString.ISNull(pCMD.GetCommandParamValue("O_STATUS")) == "F")
                            {
                                vMessage = iString.ISNull(pCMD.GetCommandParamValue("O_MESSAGE"));
                                pPM.PromptText = string.Format("Importing :: {0} / {1} *** Person Name(Allowance Item) : {2}({3} ** Error : {4})", vRow, (mTotalROW - pStartRow), vPERSON_NUM, vALLOWANCE_CODE, vMessage);
                                vERR_CNT++;
                                return false;
                            }
                        }
                    ...
                    pPB.BarFillPercent = (Convert.ToSingle(vADRow) / Convert.ToSingle(mTotalROW)) * 100F;
                    pPM.PromptText = string.Format("Importing :: {0} / {1} *** Person Name(Allowance Item) : {2}({3})", vRow, (mTotalROW - 1), vPERSON_NUM, vALLOWANCE_CODE);
        ...
        public bool LoadXL_Deduction(InfoSummit.Win.ControlAdv.ISDataCommand pCMD, int pStartRow, InfoSummit.Win.ControlAdv.ISProgressBar pPB, InfoSummit.Win.ControlAdv.ISPrompt pPM)
        {
            string vMessage = string.Empty;
            ...
            object vPERSON_NUM = string.Empty;
            object vNAME = string.Empty;
            object vDEDUCTION_CODE = string.Empty;
            object vDEDUCTION_NAME = string.Empty;
            ...
            try
            {
                for (int vRow = pStartRow; vRow < mTotalROW; vRow++)
                {
                        ...
                        iString.ISNull(vPERSON_NUM) != string.Empty)  //사원번호가 있을 경우만 처리.
                    {
                        ...
                        {
                            vNAME = mExcel_Upload.XLGetCell(vRow, 5);
                            vDEDUCTION_CODE = mExcel_Upload.XLGetCell(vRow, 6);
                            vDEDUCTION_NAME = mExcel_Upload.XLGetCell(vRow, 7);
                            ...
                            pCMD.SetCommandParamValue("P_PERSON_NUM", vPERSON_NUM);
                            pCMD.SetCommandParamValue("P_NAME", vNAME);
                            pCMD.SetCommandParamValue("P_DEDUCTION_CODE", vDEDUCTION_CODE);
                            pCMD.SetCommandParamValue("P_DEDUCTION_NAME", vDEDUCTION_NAME);
                            pCMD.SetCommandParamValue("P_DEDUCTION_AMOUNT", mExcel_Upload.XLGetCell(vRow, 8)); 
                            pCMD.SetCommandParamValue("P_DESCRIPTION", mExcel_Upload.XLGetCell(vRow, 11));
                            pCMD.ExecuteNonQuery();
                            if (iString.ISNull(pCMD.GetCommandParamValue("O_STATUS")) == "F")
                            {
                                vMessage = iString.ISNull(pCMD.GetCommandParamValue("O_MESSAGE"));
                                pPM.PromptText = string.Format("Importing :: {0} / {1} *** Person Name(Deduction Item) : {2}({3} ** Error : {4})", vRow, (mTotalROW - pStartRow), vPERSON_NUM, vDEDUCTION_CODE, vMessage);
                                vERR_CNT++;
                                return false;
                            }
                        }
                    ...
                    pPB.BarFillPercent = (Convert.ToSingle(vADRow) / Convert.ToSingle(mTotalROW)) * 100F;
                    pPM.PromptText = string.Format("Importing :: {0} / {1} *** Person Name(Deduction Item) : {2}({3})", vRow, (mTotalROW - 1), vPERSON_NUM, vDEDUCTION_CODE);
